...
export function ChecklistView({ onNavigateToTabStops }: ChecklistViewProps) {
  ...
  // Load checklist state when URL changes
  useEffect(() => {
    if (activeUrl) {
      loadForUrl(activeUrl);
    }
  }, [activeUrl, loadForUrl]);
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="rounded-lg border border-zinc-200 bg-white p-3">
        ...
        {currentUrl && (
          <div className="mt-1.5 truncate text-xs text-zinc-500" title={currentUrl}>
            {currentUrl}
          </div>
        )}
      </div>
...
function ScanAutoPopulateButton() {
  ...
  const violations = useScanStore((s) => s.violations);
  const passes = useScanStore((s) => s.passes);
  const customChecks = useScanStore((s) => s.customChecks);
  ...
  useEffect(() => {
    if (prevStatusRef.current === "scanning" && scanStatus === "complete") {
      autoPopulate(violations, passes, customChecks);
    }
    prevStatusRef.current = scanStatus;
  }, [scanStatus, violations, passes, customChecks, autoPopulate]);
  ...
  return (
    <div className="space-y-3 border-t border-zinc-100 px-3 py-3">
      ...
      <CriterionCard
        ...
        highlightTargets={highlightMap.get(currentCriterion.id) ?? []}
        onNavigateToTabStops={onNavigateToTabStops}
      />
...
function CriterionCard({
  ...
  const autoDetail = useChecklistStore((s) => s.autoDetails[criterion.id]);
  ...
  const highlightCount = highlightTargets.length;
  ...
  return (
    <div className="rounded-lg border border-zinc-200 bg-white">
      ...
      <div className="space-y-3 px-3 py-3">
        ...
        {isAuto && autoDetail && (
          <div className={`flex items-start gap-1.5 rounded border px-2 py-1.5 ${
            status === "fail"
              ? "border-red-200 bg-red-50"
              : "border-blue-200 bg-blue-50"
          }`}>
            <svg className={`mt-0.5 h-3.5 w-3.5 shrink-0 ${
            ...
            <span className={`text-xs ${
              status === "fail" ? "text-red-700" : "text-blue-700"
            }`}>
              {autoDetail}
            </span>
          </div>
        ...
        {highlightCount > 0 && !(isAuto && autoDetail) && (
        ...
        {highlightCount > 0 && (
          <HighlightedElementList targets={highlightTargets} />
        )}

        {criterion.id === "2.4.3" && highlightCount > 0 && (
          <button
...
function ElementRow({ target }: { target: HighlightTarget }) {
  ...
  }, [target.selector, target.impact]);

  return (
    <button
      onClick={handleClick}
      className="flex w-full cursor-pointer items-center gap-1.5 rounded px-2 py-1 text-left text-xs text-zinc-600 hover:bg-zinc-100"
      title={target.selector}
    >
      <svg className="h-3 w-3 shrink-0 text-zinc-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true">
        <path d="M15 3h6v6M9 21H3v-6M21 3l-7 7M3 21l7-7" />
      </svg>
      <span className="min-w-0 truncate font-mono">{target.selector}</span>
    </button>
  );
}

function HighlightedElementList({ targets }: { targets: HighlightTarget[] }) {
  const [isOpen, setIsOpen] = useState(false);

  if (targets.length === 1) {
    return (
      <div className="rounded border border-zinc-200 bg-zinc-50 py-0.5">
        <ElementRow target={targets[0]} />
      </div>
    );
  }
  ...
  return (
    <div className="rounded border border-zinc-200 bg-zinc-50">
      <button
        ...
        className="flex w-full cursor-pointer items-center gap-1 px-2 py-1.5 text-xs font-medium text-zinc-600 hover:text-zinc-800"
      >
        <svg
          className={`h-3 w-3 shrink-0 transition-transform ${isOpen ? "rotate-90" : ""}`}
          viewBox="0 0 24 24"
      ...
      {isOpen && (
        <div className="border-t border-zinc-200 py-0.5">
          {targets.map((target, i) => (
            <ElementRow key={`${target.selector}-${i}`} target={target} />
          ))}
        </div>
      )}
    </div>
