import { useScanStore } from "@/hooks/use-scan";

export function HighlightToggle() {
  const persistentHighlightOn = useScanStore((s) => s.persistentHighlightOn);
  const toggle = useScanStore((s) => s.togglePersistentHighlight);
  const status = useScanStore((s) => s.status);

  if (status !== "complete") return null;

  return (
    <button
      onClick={toggle}
      className={`flex w-full cursor-pointer items-center justify-center gap-2 rounded-lg border px-3 py-2 text-sm font-medium transition-colors ${
        persistentHighlightOn
          ? "border-red-300 bg-red-50 text-red-700"
          : "border-zinc-200 bg-white text-zinc-700 hover:bg-zinc-50"
      }`}
    >
      <svg
        className="h-4 w-4"
        viewBox="0 0 24 24"
        fill="none"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinecap="round"
        strokeLinejoin="round"
        aria-hidden="true"
      >
        {persistentHighlightOn ? (
          <>
            <path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7Z" />
            <circle cx="12" cy="12" r="3" />
          </>
        ) : (
          <>
            <path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94" />
            <path d="M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19" />
            <line x1="1" y1="1" x2="23" y2="23" />
          </>
        )}
      </svg>
      {persistentHighlightOn ? "Hide all highlights" : "Show all highlights"}
    </button>
  );
}
